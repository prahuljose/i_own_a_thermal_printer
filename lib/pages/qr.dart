...
class QrPage extends StatefulWidget {
  ...
  const QrPage({super.key, this.onConnectPressed});
...
class _QrPageState extends State<QrPage> {
  ...
  Future<void> _showTemplates() async {
    final templates = await DbService.getTemplates('qr');
    if (!mounted) return;
    ...
    showModalBottomSheet(
      ...
      builder: (_) => _TemplateSheet(
        ...
        onLoad: (dataJson) {
          ...
          _controller.text = data['data'] as String;
          setState(() {
            includeLink = data['includeText'] as bool? ?? false;
          });
        },
  ...
  Future<void> _saveTemplate() async {
    ...
    if (text.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        _snackBar("ENTER QR DATA BEFORE SAVING TEMPLATE"),
      );
      return;
    }
    ...
    await DbService.saveTemplate(
      ...
      dataJson: jsonEncode({'data': text, 'includeText': includeLink}),
    );
    if (mounted) {
          ...
          children: [
            TextField(
              ...
              decoration: InputDecoration(
                labelText: "ENTER TEXT / URL FOR QR",
            ...
            GestureDetector(
              onTap: () => setState(() => includeLink = !includeLink),
